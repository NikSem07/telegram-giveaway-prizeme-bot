...
def _compose_finished_post_text(gw: Giveaway, winners: list, participants_count: int) -> str:
    ...
    # Добавляем заголовок победителей (жирный текст)
    lines.append("<b>🎲 Розыгрыш завершен, билеты победителей определены:</b>")
    lines.append("")
    
    # Добавляем победителей ТОЛЬКО билетами, без номеров и никнеймов
    if winners:
        for winner in winners:
            rank, username, ticket_code = winner
            # ВАЖНО: показываем ТОЛЬКО билет, без номера и username
            lines.append(f"{ticket_code}")
    else:
