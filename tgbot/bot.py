...
async def cb_boost_premium(cq: CallbackQuery):
    """💎ПРЕМИУМ — без изменений (существующая логика подписки)"""
    await cq.answer()
    text = (
        "<b>Подписка дает доступ к уникальному функционалу бота:</b>\n\n"
        "🎯 Расширенные возможности для проведения розыгрышей\n"
        "📊 Детальная статистика и аналитика\n"
        "⚡️ Приоритетная поддержка\n\n"
        "<i>Оплата и управление подпиской осуществляется через сервис @tribute.</i>"
    )
    kb = InlineKeyboardBuilder()
    kb.button(text="💵 Тарифы",  url="https://t.me/tribute/app?startapp=dKpz")
...
async def cb_boost_services(cq: CallbackQuery):
    ...
    miniapp_url  = f"https://t.me/{BOT_USERNAME}/app?startapp=page_services"
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="🚀 К сервисам", callback_data="boost:services_go")
...
@dp.callback_query(F.data == "boost:services_go")
async def cb_boost_services_go(cq: CallbackQuery):
    """Подтверждение перехода в mini-app → раздел Сервисы"""
    await cq.answer()
    ...
    text = (
        "Вы будете перенаправлены в mini-app PrizeMe в раздел <b>«Сервисы»</b> режима «Создатель».\n\n"
        "Нажмите <b>«Перейти»</b> для продолжения."
    )
    kb = InlineKeyboardBuilder()
    kb.button(text="🚀 Перейти", url=miniapp_url)
    kb.button(text="Отмена",     callback_data="boost:services")
    kb.adjust(1)
    await cq.message.edit_text(text, reply_markup=kb.as_markup(), parse_mode="HTML")
...
async def cb_boost_donate(cq: CallbackQuery):
    """❤️Донат — без изменений"""
    await cq.answer()
    # Существующая логика доната остаётся как есть
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="boost:back")
    kb.adjust(1)
    await cq.message.edit_text(
        "❤️ <b>Донат</b>\n\nСпасибо за желание поддержать проект! Детали скоро появятся.",
        reply_markup=kb.as_markup(),
        parse_mode="HTML"
    )
